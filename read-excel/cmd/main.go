...
func main() {
	...
	db.Find(&res)
	// fmt.Println(res)
	newRes := reverseTree(res)
	fmt.Printf("newRes:%v\n", newRes)
	b, _ := json.Marshal(newRes) //序列号
...
func reverseTree(data []*BusinessKnowledge) []*BusinessKnowledge {
	if data == nil || len(data) == 0 {
		return []*BusinessKnowledge{}
	}

	mapComment := make(map[uint64]*BusinessKnowledge, len(data))
	ret := []*BusinessKnowledge{}
	for _, c := range data {
		c.Child = []*BusinessKnowledge{}
		if c.Business_pid == 0 {
			ret = append(ret, c)
		}
		mapComment[uint64(c.Business_id)] = c
	}
	for _, c := range data {
		if c.Business_pid != 0 {
			parent := mapComment[uint64(c.Business_pid)]
			c.Child = []*BusinessKnowledge{}
